...
function MapView() {
  ...
  const [analyzedTiles, setAnalyzedTiles] = useState(new Set());
  ...
  const recordTileAnalysis = async (tileId, mode) => {
    try {
      await fetch(`${API_BASE}/auth/history`, {
        method: "POST",
        headers: {
      ...
      // Update local state to track analyzed tiles
      setAnalyzedTiles(prev => new Set([...prev, `${tileId}-${mode}`]));
    } catch (error) {
      console.error("Failed to record tile analysis:", error);
    }
  };

  useEffect(() => {
    // Record analysis when user views a tile
    if (activeTile && viewMode) {
      const analysisKey = `${activeTile.tile_id}-${viewMode}`;
      if (!analyzedTiles.has(analysisKey)) {
        recordTileAnalysis(activeTile.tile_id, viewMode);
      }
    }
  }, [activeTile, viewMode]);
            ...
            {activeTile && (
              <>
                <div style={{ marginBottom: "8px" }}>
                  ...
                  {analyzedTiles.has(`${activeTile.tile_id}-${viewMode}`) && (
                    <span style={{ 
                      marginLeft: "8px", 
                      fontSize: "12px", 
                      color: "#28a745",
